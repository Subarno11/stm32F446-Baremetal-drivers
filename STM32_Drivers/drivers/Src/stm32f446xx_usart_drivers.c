...
void USART_ClearFlag(USART_RegDef_t *pUSARTx, uint16_t FlagName){
	volatile uint32_t dummyRead;
		...
		if(FlagName == USART_FLAG_PE || FlagName == USART_FLAG_FE || FlagName == USART_FLAG_NF || FlagName == USART_FLAG_ORE|| FlagName == USART_FLAG_IDLE ){
			dummyRead = pUSARTx->USART_SR;
			dummyRead= pUSARTx->USART_DR;
		}

		else if(FlagName == USART_FLAG_CTS || FlagName == USART_FLAG_LBD || FlagName == USART_FLAG_TC){
...
//The blow code will get executed in only if multibuffer mode is used.
	...
	if(temp2 )
	{
		...
		if(temp1 & ( 1 << USART_SR_FE))
		{
			...
			*/
			USART_ClearFlag(pUSARTHandle->pUSARTx, USART_FLAG_FE);
		...
		if(temp1 & ( 1 << USART_SR_NF) )
		{
			/*
				...
				USART_DR register).
			*/
			USART_ClearFlag(pUSARTHandle->pUSARTx, USART_FLAG_NF);
...
__weak void USART_ApplicationEventCallback(USART_Handle_t *pUSARTHandle,uint8_t AppEv){
	//This is a weak implementation API
}
