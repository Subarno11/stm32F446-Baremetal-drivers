...
int main(){
	...
	while(1){
		...
		rxComplt = RESET;
		while(I2C_MasterRecieveDataIT(&I2C1Handle, &len, 1, SLAVE_ADDR , I2C_ENABLE_SR)  != I2C_READY);
		while(rxComplt != SET);
		...
		rxComplt = RESET;
		while(I2C_MasterRecieveDataIT(&I2C1Handle, rcv_Buffer, len, SLAVE_ADDR , I2C_DISABLE_SR) != I2C_READY);
